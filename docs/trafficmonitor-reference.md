...
double-line custom item its own column, which is why the IPv4 and IPv6 daily totals
remain visually grouped in one compact taskbar column.
...
accumulates daily deltas for each family, persisting the current local date and totals
in the plugin configuration directory. This intentionally documents the measurement scope:
